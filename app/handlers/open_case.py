...
from app.texts.ua import CASE_LOCKED_CP, NO_CASE_FOUND, OPEN_CASE_NOT_ENOUGH, OPEN_CASE_SUCCESS
...
async def cmd_open_case(message: Message) -> None:
    ...
    if len(parts) < 2:
        await message.answer("Вкажи код кейсу: /open_case classic")
        return

    case_code = parts[1].strip()
