...
async def buy_listing(session: AsyncSession, buyer: User, listing: MarketListing) -> MarketListing:
    ...
    if listing.user_item.item.rarity is ItemRarity.MYTHIC:
...
async def sell_to_bot(session: AsyncSession, user: User, user_item: UserItem) -> int:
    ...
    if user_item.item.rarity is ItemRarity.MYTHIC:
...
async def load_active_listings(session: AsyncSession, limit: int = 10, offset: int = 0) -> Sequence[MarketListing]:
