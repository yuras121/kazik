...
async def open_case(session: AsyncSession, user: User, case: Case) -> CaseResult:
    ...
    await economy.withdraw(session, user, case.price, TransactionKind.CASE_PURCHASE, meta={"case": case.code})
    ...
    for entry in case_item_rows:
        ...
        if pity_level > 0 and rarity in {ItemRarity.EPIC, ItemRarity.LEGENDARY, ItemRarity.MYTHIC}:
    ...
    pity_reset = item.rarity in {ItemRarity.EPIC, ItemRarity.LEGENDARY, ItemRarity.MYTHIC}
    if pity_reset:
        user.pity_counter = 0
    else:
        user.pity_counter += 1
